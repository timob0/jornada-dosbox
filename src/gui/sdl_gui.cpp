...
static GUI::ScreenSDL *UI_Startup(GUI::ScreenSDL *screen) {
	...
	SDL_Surface* sdlscreen = SDL_SetVideoMode(w, 240, 32, SDL_SWSURFACE|(fs?SDL_FULLSCREEN:0));
	...
	for (int i = 0xff; i > 0; i -= 0x11) {
		...
		SDL_BlitScaled(background, NULL, sdlscreen, NULL);
		SDL_BlitScaled(screenshot, NULL, sdlscreen, NULL);
